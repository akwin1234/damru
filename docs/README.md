...
*   **CLI setup**: Run `python -m damru setup` for guided first-run configuration, `python -m damru install-deps` for Linux/WSL dependencies, `python -m damru install-image` to load the baked Redroid image, `python -m damru install-apks --download` for raw Chrome/WebView/TTS APK assets when baking or using unbaked Redroid, `python -m damru fix-wsl` for safe Docker/binderfs/netfilter repair, and `python -m damru check-env` to verify Docker, binderfs, image/assets, and the Damru Playwright `crPage.js` patch. WSL setup keeps Docker/Redroid inside WSL and prefers legacy iptables; native Linux prefers nft iptables to match modern Docker daemons.
